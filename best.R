...
best <- function(state, outcome){
        ...
        {
                i <- i + 1
                data_outcome <- data_outcome[ order(data_outcome[,3], data_outcome[,2]), ]        
        }
        ## Order the dataset by outcome and state
        if (outcome == "heart failure")
        {
                i <- i + 1
                data_outcome <- data_outcome[ order(data_outcome[,4], data_outcome[,2]), ]
        }
        if (outcome == "pneumonia")
        {
                i <- i + 1
                data_outcome <- data_outcome[ order(data_outcome[,5], data_outcome[,2]), ]
        }
